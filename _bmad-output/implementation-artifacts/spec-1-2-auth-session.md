...
- O bootstrap recupera a sessão pelo BFF; somente `401` confirma sessão inválida e limpa a memória. Rede, `429`, `5xx` e `403 AUTH_ORIGIN_REJECTED` mantêm a rota protegida bloqueada, repetem uma vez e oferecem tentativa manual sem publicar logout.
- O middleware usa somente a presença do cookie de refresh como pré-filtro de navegação e nunca como autorização.
...
- matriz terminal/transitória do refresh, retry limitado, tentativa manual e resposta tardia após unmount;
- mensagem de logout entre abas sem propagação para falha transitória;
