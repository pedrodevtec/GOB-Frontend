...
| Participante novo | E-mail ainda não confirmado; sem consentimento e sem vínculo | cadastrar; confirmar e-mail; `GET /campaigns/public/pilot-v1/resume`; aceitar consentimento; entrar; ler contexto público; `POST /campaigns/public/pilot-v1/character-draft` | resposta contém contexto `PUBLISHED` somente com unidades `PUBLIC`, `Character.id`, configuração e revisão; cache atualizado antes da navegação; `CHARACTER_DRAFT`; `/campanhas/pilot-v1/personagem` | NÃO EXECUTADO |
| Consentimento versionado e atômico | participante autenticado sem aceite; repetir também com duas abas e versão alterada | `GET /campaigns/public/pilot-v1/consent`; enviar `status`, versão exibida e source; repetir envio concorrente; alterar versão antes do POST | exatamente um aceite e uma membership; divergência retorna `409`, mantém a tela e exige releitura; sucesso segue para contexto público | NÃO EXECUTADO |
| Recusa e revogação | participante sem aceite e participante ativo | registrar `DECLINED`; depois acessar `?gerenciar=1`, confirmar `REVOKED` e tentar ação protegida | recusa não cria membership; revogação remove acesso, despublica conteúdo aplicável e pode encerrar a sessão | NÃO EXECUTADO |
...
| Reload com sessão válida (#60) | login novo e cookie válido; repetir em desktop e mobile | abrir uma rota protegida, anotar a URL e pressionar F5; observar status e `error.code` de `POST /api/auth/refresh` sem registrar cookie/token | usuário e access token voltam à memória, conteúdo só aparece depois da confirmação e a URL original é preservada | NÃO EXECUTADO |
| Falha temporária no reload (#60) | sessão válida; refresh controlado para rede, `429`, `503` e `403 AUTH_ORIGIN_REJECTED` | recarregar rota protegida; conferir uma repetição automática, tela bloqueada e ação `Tentar novamente`; restaurar o serviço e acionar a tentativa | nenhum logout ou redirecionamento; nenhuma superfície privada aparece antes da restauração; tentativa manual preserva a rota | NÃO EXECUTADO |
| Reload e logout em duas abas (#60) | mesma sessão válida em duas abas protegidas | recarregar as duas abas; simular falha temporária em uma; depois retornar `401` terminal em uma aba | falha temporária não afeta a outra aba; `401` limpa uma vez, propaga logout e ambas preservam somente `returnTo` interno | NÃO EXECUTADO |
...
| Retomar rascunho | `DRAFT` persistido | sair; entrar novamente; abrir Minha Jornada; `GET .../resume`; repetir `POST /campaigns/public/pilot-v1/character-draft` em duas abas e com retry após falha de navegação | todas as respostas retornam o mesmo `Character.id`; existe um único personagem no banco; mesmos dados e revisão; builder no ponto salvo | NÃO EXECUTADO |
