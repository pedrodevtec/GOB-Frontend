...
import { AlertTriangle, CheckCircle2, ChevronDown, Search, UserRound } from "lucide-react";
...
type ParticipantFilter = "all" | "attention" | "building" | "review" | "complete";

interface ParticipantStage {
  label: string;
  helper: string;
  group: Exclude<ParticipantFilter, "all">;
...
export function PilotParticipantsPanel({ slug }: { slug: string }) {
  ...
  const visible = useMemo(() => {
    ...
    return participants.filter((participant) => {
      const matchesFilter = filter === "all" || stageFor(participant).group === filter;
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {(Object.keys(filterLabels) as ParticipantFilter[]).map((key) => (
          <button key={key} type="button" onClick={() => setFilter(key)} className={cn("rounded-2xl border p-4 text-left transition", filter === key ? "border-primary/50 bg-primary/10" : "border-white/10 bg-black/20 hover:bg-white/5")}>
      ...
      <Card className="p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="text-sm text-muted-foreground">{visible.length} de {participants.length} participante(s)</p>
        </div>
      </Card>

      <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
        <div className="hidden grid-cols-[minmax(240px,1.4fr)_minmax(180px,1fr)_minmax(200px,1fr)_auto] gap-4 border-b border-white/10 px-4 py-3 text-xs uppercase tracking-wide text-muted-foreground lg:grid">
          <span>Participante</span><span>Personagem</span><span>Etapa atual</span><span>Ação</span>
        </div>
        {visible.length ? visible.map((participant) => {
          const stage = stageFor(participant);
          const character = participant.character;
          const isExpanded = expanded === participant.membershipId;
          return (
            <div key={participant.membershipId} className="border-b border-white/10 last:border-b-0">
                ...
                <div className="border-t border-white/10 bg-slate-950/35 px-4 py-4">
                  ...
                  {deleteTarget === character?.id ? (
                    <form className="mt-4 space-y-3 rounded-xl border border-destructive/30 bg-destructive/10 p-3" onSubmit={(event) => { event.preventDefault(); const reason = deleteReason.trim(); if (!reason) return; deleteCharacter.mutate({ characterId: character.id, reason }, { onSuccess: () => { setDeleteTarget(null); setDeleteReason(""); } }); }}>
                      ...
                      <textarea required value={deleteReason} onChange={(event) => setDeleteReason(event.target.value)} rows={2} className="flex w-full rounded-xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm outline-none focus:border-primary" placeholder="Motivo da exclusão" />
...
function Status({ label, value, ok }: { label: string; value: string; ok: boolean }) {
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs uppercase tracking-wide text-muted-foreground">{label}</p>{ok ? <CheckCircle2 className="h-4 w-4 text-emerald-300" /> : <AlertTriangle className="h-4 w-4 text-amber-300" />}</div><p className="mt-2 text-sm font-medium">{value}</p></div>;
}
